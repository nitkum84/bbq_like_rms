...
<div class="page-header">
    <h1 class="page-title">Edit Booking: {{ $booking->confirmation_code ?? '#'.$booking->id }}</h1>
    <a href="{{ route('admin.bookings.show', $booking) }}" class="btn btn-outline-secondary"><i class="bi bi-arrow-left me-1"></i>Back</a>
</div>
        ...
        <div class="admin-card">
            ...
            <div class="admin-card-body">
                <form action="{{ route('admin.bookings.update', $booking) }}" method="POST">
                    @csrf
                    @method('PUT')
                    @include('admin.bookings._form', ['submitLabel' => 'Update Booking'])
                </form>
            </div>
        </div>
    </div>
